...
ExpressiveAuth.prototype.OAUTH2VerifyCallback = function(accessToken, refreshToken, profile, done){
    ...
    var credentials = {domain: profile.provider, type:'OAUTH2', accessToken:accessToken, refreshToken:refreshToken};
    
    self.User.findByDomainAccount(profile.provider, profile.id, function(err, user){
...
ExpressiveAuth.prototype.localVerifyCallback = function(username, password, done){
    var self = this;
    self.User.findByLocalCredentials(username, password, function(err, user){
